-- Very useful utility library
-- https://github.com/incinirate/luascore
...
local function getFunctionArgs(func)
  if debugInfo then
    ...
    error("Debug library is disabled", 3)
  end
end

-- Unsafe because it relies on debug library
function _.unsafeCurry(f)
  local argCount = #getFunctionArgs(f)

  return _.curry(f, argCount)
end

function _.curry(f, argCount)
  local courier
  courier = function(...)
    if select("#", ...) >= argCount then
      return f(...)
    else
      local oargs = tablePack(...)
      return function(...)
        return courier(tableUnpack(_.push(oargs, ...)))
      end
    end
  end
...
function _.partial(f, ...)
  ...
  return function(...)
    ...
    return f(tableUnpack(actual, 1, k))
  end
end
...
function _.zip(...)
  ...
  return _.map(args[1], function(...) return {...} end, tableUnpack(args, 2, args.n))
end
...
function _.zipOnKey(...)
  ...
  return _.mapWithKey(args[1], function(x) return x, _.map(args, x) end)
end
